...
// Set SECRETS_SCAN_ENABLED=false as a site env var
const envVar = [{
  key: 'SECRETS_SCAN_ENABLED',
  values: [{ value: 'false', context: 'all' }]
}];

const r = await apiCall('POST', `/api/v1/accounts/zaio/env?site_id=${SITE_ID}`, envVar);
console.log('Set SECRETS_SCAN_ENABLED=false:', r.s, JSON.stringify(r.b).substring(0, 100));
